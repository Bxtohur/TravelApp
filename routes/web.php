...
Route::get('/dashboard', function () {
    ...
    if ($user->role === 'admin' || $user->role === 'owner') {
        ...
        $recentTransactions = Transaction::with(['user', 'package'])->latest()->take(5)->get();
...
Route::middleware(['auth'])->prefix('admin')->name('admin.')->group(function () {
    ...
    // Validasi Transaksi
    Route::resource('transactions', AdminTransactionController::class)->only(['index', 'update', 'show']);
});
...
Route::middleware(['auth'])->prefix('customer')->name('customer.')->group(function () {
    ...
    Route::get('/package/{package:slug}', [CustomerPackageController::class, 'show'])->name('packages.show');

    // 2. Proses Booking (Simpan Pesanan)
    Route::post('/book/{package}', [CustomerTransactionController::class, 'store'])->name('transactions.store');

    // 3. Riwayat & Bayar
    Route::get('/transactions', [CustomerTransactionController::class, 'index'])->name('transactions.index'); // List Pesanan
    Route::get('/transactions/{transaction}', [CustomerTransactionController::class, 'show'])->name('transactions.show'); // Halaman Bayar
    Route::patch('/transactions/{transaction}', [CustomerTransactionController::class, 'update'])->name('transactions.update'); // Upload Bukti
});
